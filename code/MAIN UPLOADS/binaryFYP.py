...
def setup_unique_output_file(base_dir, base_name):
    ...
    while True: # loop until a unique filename is found
        ...
        if not os.path.exists(print_path): # if the file does not exist, break the loop
            break
        count += 1
...
# define the custom dataset class for the binary classification task
class CustomDataset(Dataset):
    def __init__(self, data_folder, labels_csv, transform=None):
        ...
        # change mapping to face and non-face
        self.label_mapping = {
            'face': 0,
            'non-face': 1
        }
...
with torch.no_grad():
    ...
    tpr = dict()
    roc_auc = dict()
    # convert all_scores to a numpy array
...
# print average performance metrics for the model
...
for i, (class_name, prec, rec, f1_score) in enumerate(zip(class_names, precision, recall, f1)):
    ...
    for j, other_class in enumerate(class_names):
        if i != j:
            logging.info(f"  Misclassified as '{other_class}': {cm[i, j]}")
...
# Save the model
torch.save(model.state_dict(), 'C://Users//zeesh//Downloads//ALLFYPDATASETS//4CLASSSPLITDATA//FINAL_MODEL_FYP.pth')